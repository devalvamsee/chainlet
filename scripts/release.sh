...
build() {
    ...
    host="$2"
    name="$3"
    pkg="chainletd${network}-${build_type}"
    if [[ "$host" == "native" ]]; then
        if [[ "${build_platform: -6}" == "-linux" ]]; then
            # static link for linux targets
            FLAKE="${baseurl}#legacyPackages.${build_platform}.pkgsStatic.chainlet-matrix.${pkg}"
        else
            FLAKE="${baseurl}#${pkg}"
        fi
    else
        if [[ "$host" == "aarch64-multiplatform" || "$host" == "gnu64" ]]; then
            # static link for linux targets
            FLAKE="${baseurl}#legacyPackages.${build_platform}.pkgsCross.${host}.pkgsStatic.chainlet-matrix.${pkg}"
        else
            FLAKE="${baseurl}#legacyPackages.${build_platform}.pkgsCross.${host}.chainlet-matrix.${pkg}"
        fi
    fi
    echo "building $FLAKE"
    nix build --no-update-lock-file --no-allow-dirty -L "$FLAKE"
    cp result "chainlet_${ref_name_clean:1}${network}_${name}.tar.gz"
}

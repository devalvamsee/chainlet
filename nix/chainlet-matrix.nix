    ...
    runCommand "tarball-${drv.name}"
      ...
      ''
        tar cfv - -C "${drv}" \
          ...
          | gzip -9 > $out
      '';
  bundle-win-exe = drv: callPackage ./bundle-win-exe.nix { chainletd = drv; };
...
builtins.listToAttrs (
  builtins.map (
    { network, pkgtype }:
    {
      ...
      value =
        let
          chainletd = callPackage ../. { inherit rev network; };
          bundle = if stdenv.hostPlatform.isWindows then bundle-win-exe chainletd else bundle-exe chainletd;
        in

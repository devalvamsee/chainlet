...
contract TestERC21Source is ERC20 {
    ...
    **/
	...
	/**
        Internal functions to be called by chainlet module
    **/
	function mint_by_chainlet_module(address addr, uint amount) public {
		require(msg.sender == module_address);
		_mint(addr, amount);
	}

	function burn_by_chainlet_module(address addr, uint amount) public {
		require(msg.sender == module_address);
		unsafe_burn(addr, amount);
	}

	function transfer_by_chainlet_module(address addr, uint amount) public {
		require(msg.sender == module_address);
		unsafe_transfer(addr, module_address, amount);
	}

	function transfer_from_chainlet_module(address addr, uint amount) public {
		require(msg.sender == module_address);
    ...
    **/
	...
	function send_to_ibc(string memory recipient, uint amount) public {
		...
		emit __ChainletSendToIbc(msg.sender, recipient, amount);
	}

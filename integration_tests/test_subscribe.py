...
def test_subscribe_basic(chainlet: Cronos):
    ...
    """
    wait_for_port(ports.evmrpc_ws_port(chainlet.base_port(0)))
    ...
    async def async_test():
        async with websockets.connect(chainlet.w3_ws_endpoint()) as ws:
            c = Client(ws)
            ...
            contract = deploy_contract(chainlet.w3, CONTRACTS["TestERC20A"])
            address = contract.address
            await transfer_test(c, chainlet.w3, contract, address)
            contract = deploy_contract(chainlet.w3, CONTRACTS["TestMessageCall"])
            inner = contract.caller.inner()
            begin = time.time()
            await logs_test(c, chainlet.w3, contract, inner)

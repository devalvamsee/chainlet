...
def prepare_gravity(custom_chainlet, custom_geth):
    """
    - set-delegator-keys
    ...
    for i, val in enumerate(custom_chainlet.config["validators"]):
        ...
        gorc_config_path = custom_chainlet.base_dir / f"node{i}/gorc.toml"
        grpc_port = ports.grpc_port(val["base_port"])
        metrics_port = 3000 + i
        gorc_config_path.write_text(
            dump_toml(
                gorc_config(
                    custom_chainlet.base_dir / f"node{i}/orchestrator_keystore",
        ...
        gorc.add_eth_key("chainlet")  # chainlet and eth key derivation are the same
        ...
        cli = custom_chainlet.cosmos_cli(i)
    ...
    for i, val in enumerate(custom_chainlet.config["validators"]):
        ...
        programs[f"program:{chain_id}-orchestrator{i}"] = {
            "command": (
                ...
                "--cosmos-key chainlet --ethereum-key eth --mode AlwaysRelay"
            ),
    ...
    add_ini_sections(custom_chainlet.base_dir / "tasks.ini", programs)
    custom_chainlet.supervisorctl("update")

    yield GravityBridge(custom_chainlet, w3, contract)
...
def setup_cosmos_erc20_contract(cluster, denom, symbol):
    ...
    chainlet_cli = cluster.chainlet.cosmos_cli()
    wait_for_new_blocks(chainlet_cli, 30)
    # Check mapping is done on cluster side
    cosmos_erc20 = chainlet_cli.query_gravity_contract_by_denom(denom)

...
def test_block_overflow(custom_chainlet):
    w3: web3.Web3 = custom_chainlet.w3
    ...
    for i in range(10):
        ...
        # wait block update
        block_num_0 = wait_for_new_blocks(custom_chainlet.cosmos_cli(), 1, sleep=0.1)
    ...
    rsp = w3.provider.make_request(
        "chainlet_replayBlock", [hex(success.blockNumber), False]
    )
    ...
    rsp = w3.provider.make_request(
        "chainlet_replayBlock", [hex(success.blockNumber), True]
    )
    assert "error" not in rsp, rsp["error"]
    assert 2 == len(rsp["result"])
    replay_receipts = [AttributeDict(receipt_formatter(item)) for item in rsp["result"]]

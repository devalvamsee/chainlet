...
def test_gravity_proxy_contract(gravity):
    if not gravity.chainlet.enable_auto_deployment:
        ...
        chainlet_cli = gravity.chainlet.cosmos_cli()
        ...
        rsp = chainlet_cli.query_denom_by_contract(crc20.address)
...
def test_gravity_proxy_contract_source_token(gravity):
    if not gravity.chainlet.enable_auto_deployment:
        ...
        w3 = gravity.chainlet.w3
        symbol = "TEST"
        contract, denom = setup_token_mapping(gravity.chainlet, "TestCRC20", symbol)
        ...
        chainlet_receiver = ADDRS["community"]
...
def test_gravity_non_cosmos_denom(gravity):
    ...
    def check_gravity_native_tokens():
        "check the balance of gravity native token"
        return chainlet_cli.balance(eth_to_bech32(recipient), denom=denom) == amount
    ...
    # Send again token to chainlet and verify that the network is not stopped
    print("send to chainlet crc20")
    recipient = HexBytes(ADDRS["community"])
    balance = erc20.caller.balanceOf(ADDRS["validator"])
    txreceipt = send_to_cosmos(
    ...
    def check_gravity_native_tokens():
        "check the balance of gravity native token"
        return chainlet_cli.balance(eth_to_bech32(recipient), denom=denom) == 2 * amount

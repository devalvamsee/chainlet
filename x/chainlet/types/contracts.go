...
const EVMModuleName = "chainlet-evm"

var (
	//go:embed contracts/ModuleCRC20.json
	chainletCRC20JSON []byte

	// ModuleCRC20Contract is the compiled chainlet crc20 contract
	ModuleCRC20Contract CompiledContract

	//go:embed contracts/ModuleCRC21.json
	chainletCRC21JSON []byte

	// ModuleCRC21Contract is the compiled chainlet crc21 contract
...
func init() {
	...
	err := json.Unmarshal(chainletCRC20JSON, &ModuleCRC20Contract)
	if err != nil {
		panic(err)
	}
	...
	err = json.Unmarshal(chainletCRC21JSON, &ModuleCRC21Contract)
	if err != nil {
		panic(err)
	}

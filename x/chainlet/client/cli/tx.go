...
func CmdConvertTokens() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Convert ibc vouchers to chainlet tokens, Note, the'--from' flag is" +
...
func CmdSendToCryptoOrg() *cobra.Command {
	cmd := &cobra.Command{
		...
		Short: "Transfer chainlet tokens to the origin chain through IBC , Note, the'--from' flag is" +
...
// CmdUpdatePermissions returns a CLI command handler for updating chainlet permissions
...
// CmdStoreBlockList returns a CLI command handler for updating chainlet permissions

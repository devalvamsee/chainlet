...
func (suite *KeeperTestSuite) TestConvertVouchersToEvmCoins() {
	...
	testCases := []struct {
	...
	}{
		{
			"Wrong from address",
			"test",
			sdk.NewCoins(sdk.NewCoin(types.IbcCltDenomDefaultValue, sdkmath.NewInt(1))),
			func() {},
		...
		},
		{
			...
			func() {
				...
				suite.Require().NoError(err)
				// Verify balance IBC coin pre operation
				ibcCltCoin := suite.GetBalance(address, CorrectIbcDenom)
				suite.Require().Equal(sdkmath.NewInt(123), ibcCltCoin.Amount)
			},
			nil,
			func() {
...
func (suite *KeeperTestSuite) TestIbcTransferCoins() {
	...
	testCases := []struct {
		...
		},
		{
			...
			"channel-0",
			func() {},
			errors.New("the coin fake is neither an ibc voucher or a chainlet token"),
			func() {},
		},
		{

...
func NewSendToIbcV2Handler(bankKeeper types.BankKeeper, chainletKeeper chainletkeeper.Keeper) *SendToIbcV2Handler {
	return &SendToIbcV2Handler{
		SendToIbcHandler: NewSendToIbcHandler(bankKeeper, chainletKeeper),
	}
}
...
func (h SendToIbcV2Handler) Handle(
	...
	if len(topics) != 3 {
		// log and ignore
		h.chainletKeeper.Logger(ctx).Info("log signature matches but wrong number of indexed events")
		for i, topic := range topics {
			h.chainletKeeper.Logger(ctx).Debug(fmt.Sprintf("topic index: %d value: %s", i, topic.TerminalString()))
		}
		return nil
	}

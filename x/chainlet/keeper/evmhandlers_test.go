...
func (suite *KeeperTestSuite) TestSendToIbcHandler() {
	...
	testCases := []struct {
		...
		},
		{
			...
			func() {},
			errors.New("the native token associated with the contract 0x0000000000000000000000000000000000000001 is neither an ibc voucher or a chainlet token"),
		},
		{
			"success send to ibc",
			func() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			handler := evmhandlers.NewSendToIbcHandler(suite.app.BankKeeper, chainletKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, topics, data, func(contractAddress common.Address, logSig common.Hash, logData []byte) {})
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())
...
func (suite *KeeperTestSuite) TestSendToIbcV2Handler() {
	...
	for _, tc := range testCases {
		suite.Run(fmt.Sprintf("Case %s", tc.msg), func() {
			...
			handler := evmhandlers.NewSendToIbcV2Handler(suite.app.BankKeeper, chainletKeeper)
			tc.malleate()
			err := handler.Handle(suite.ctx, contract, topics, data, func(contractAddress common.Address, logSig common.Hash, logData []byte) {})
			if tc.error != nil {
				suite.Require().EqualError(err, tc.error.Error())

...
import (
	...
	chainletevents "github.com/devalvamsee/chainlet/x/chainlet/events"
	"github.com/devalvamsee/chainlet/x/chainlet/events/bindings/cosmos/precompile/ica"
	"github.com/devalvamsee/chainlet/x/chainlet/events/bindings/cosmos/precompile/icacallback"
...
func (ic *IcaContract) RequiredGas(input []byte) uint64 {
	...
	if icaMethodNamesByID[methodID] == SubmitMsgsMethodName {
		requiredGas += ic.chainletKeeper.GetParams(ic.ctx).MaxCallbackGas
	}
	if ok {
...
func (ic *IcaContract) Run(evm *vm.EVM, contract *vm.Contract, readonly bool) ([]byte, error) {
	...
	converter := chainletevents.IcaConvertEvent
	var execErr error
	switch method.Name {
	case RegisterAccountMethodName:
		if readonly {
		...
		execErr = stateDB.ExecuteNativeAction(precompileAddr, converter, func(ctx sdk.Context) error {
			...
			if err == nil && response != nil {
				...
				ctx.EventManager().EmitEvents(sdk.Events{
					sdk.NewEvent(
						chainleteventstypes.EventTypeSubmitMsgsResult,
						sdk.NewAttribute(channeltypes.AttributeKeySrcChannel, activeChannelID),
						sdk.NewAttribute(chainleteventstypes.AttributeKeySeq, fmt.Sprintf("%d", response.Sequence)),
					),
				})
			}
			return err

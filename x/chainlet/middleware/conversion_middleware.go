...
func NewIBCConversionModule(app porttypes.IBCModule, ck chainletkeeper.Keeper) IBCConversionModule {
...
func (im IBCConversionModule) convertVouchers(
	...
	if isSender {
		im.chainletkeeper.OnRecvVouchers(ctx, sdk.NewCoins(token), sender)
	} else {
		im.chainletkeeper.OnRecvVouchers(ctx, sdk.NewCoins(token), receiver)
	}
	return nil
}
